...
function convertBelowThousand(number) {
  ...
  const tail = remainder ? ` ${convertBelowHundred(remainder)}` : ''
  return `${ones[hundred]} Hundred${tail}`
}
...
export function numberToIndianWords(value) {
  ...
  const rupeeWords = convertIndian(rupees)
  if (!paise) return `${rupeeWords} Rupees Only`
