...
function App() {
  ...
  useEffect(() => {
    if (!visibleModules.some((module) => module.id === activeModule)) {
      setActiveModule(visibleModules[0]?.id || 'dashboard')
    }
  ...
  if (!currentUser) {
    return (
      <div className="make-auth-shell">
        {renderLanguageToggle('language-toggle-login')}
  ...
  return (
    ...
    >
      <section className="make-app-header-row">
        ...
        <div className="make-header-actions">
          ...
          <div className="make-user-block">
            ...
            <small>{mandirProfile.address || MANDIR_PROFILE.address}</small>
          </div>
          <button type="button" className="make-chip-btn" onClick={handleLogout}>
            &#10140; Logout
          </button>
        </div>
      </section>
      ...
      <div className="make-workspace-layout">
        <aside className="make-sidebar">
          {sidebarModules.map((module) => (
            <button
              key={module.id}
              type="button"
              className={activeModule === module.id ? 'make-side-link active' : 'make-side-link'}
        ...
        <main className="make-main-panel">
          {notice.text && <div className={`notice ${notice.type}`}>{notice.text}</div>}
