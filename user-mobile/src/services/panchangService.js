...
function deriveApproxTithi(date) {
  if (!(date instanceof Date) || Number.isNaN(date.getTime())) return null

  const daysSinceKnownNewMoon = (date.getTime() - KNOWN_NEW_MOON_UTC_MS) / DAY_IN_MS
  const moonAge = ((daysSinceKnownNewMoon % SYNODIC_MONTH_DAYS) + SYNODIC_MONTH_DAYS) % SYNODIC_MONTH_DAYS
  const absoluteTithiNumber = Math.floor((moonAge / SYNODIC_MONTH_DAYS) * 30) + 1
  const isShuklaPaksha = absoluteTithiNumber <= 15
  const paksha = isShuklaPaksha ? 'Shukla Paksha' : 'Krishna Paksha'
  const pakshaTithiNumber = isShuklaPaksha ? absoluteTithiNumber : absoluteTithiNumber - 15

  if (pakshaTithiNumber === 15) {
    return {
      tithi: isShuklaPaksha ? 'Poonam' : 'Amavasya',
      paksha,
    }
  }

  return {
    tithi: TITHI_NAME_BY_NUMBER[pakshaTithiNumber] || 'Not Available',
    paksha,
  }
}
...
function enrichRecord(record, dateOverride) {
  ...
  const approxTithi = resolvedDate ? deriveApproxTithi(resolvedDate) : null
  const shouldUseApproxTithi = Boolean(approxTithi && isoDate && !JANUARY_2026_EXACT_BY_DATE.has(isoDate))
  const nextRecord = {
    ...record,
    date: isoDate,
    tithi: shouldUseApproxTithi ? approxTithi.tithi : record.tithi || 'Not Available',
    paksha: shouldUseApproxTithi ? approxTithi.paksha : record.paksha || 'Not Available',
    jainMonth: record.jainMonth || '-',
    festival: record.festival || '',
    kalyanak: record.kalyanak || '',
  }

  if (shouldUseApproxTithi && !record.festival && !record.kalyanak) {
    nextRecord.auspiciousInfo = ''
    nextRecord.rituals = ''
  }

  nextRecord.lunarDate = record.lunarDate || buildLunarDate(nextRecord)
  if (shouldUseApproxTithi) {
    const hasFestivalOrKalyanakFasting = Boolean((record.festival || record.kalyanak) && normalizeText(record.fasting))
    nextRecord.fasting = hasFestivalOrKalyanakFasting ? record.fasting : buildDefaultFasting(nextRecord)
  } else {
